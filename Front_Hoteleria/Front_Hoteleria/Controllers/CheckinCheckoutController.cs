...
namespace Front_Hoteleria.Controllers
{
    public class CheckinCheckoutController : Controller
    {
        ...
        public async Task<ActionResult> Tabla(DateTime? fecha, int idEstado)
        {
            var token = GetBearer();
            if (string.IsNullOrWhiteSpace(token))
                return new HttpStatusCodeResult((int)HttpStatusCode.Unauthorized, "Sesión expirada.");

            var lista = await _api.ListarReservasAsync(fecha, idEstado, token)
        ...
        {
            var token = GetBearer();
            var kpi = await _api.KpiAsync(DateTime.Today, token)
                      ?? new CheckinKpiDto();

            return PartialView("~/Views/CheckinCheckout/_DashboardCheckin.cshtml", kpi);
        }
